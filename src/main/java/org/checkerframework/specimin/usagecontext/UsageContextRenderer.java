...
/**
 * Renders a declaration's usage-context evidence as a human-readable text block, for the
 * {@code AMBIGUOUS_NEEDS_LLM} case in {@link UsageContextInference}: when the checker's own
 * evidence is mixed (some usage sites are SAFE, some UNSAFE), this prints every site instead of
 * guessing, so the LLM inference step can weigh them.
 */
